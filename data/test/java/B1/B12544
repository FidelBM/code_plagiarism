...
public class Qualifier2012 {
	public static void main(String[] args) {
		ContestProblem problem = codeJam2012_Qualification_C;
		File folder = new File("C:\\Users\\Admin\\Desktop\\GoogleJam Tests\\2012\\Qualification Round\\");

		File test = new File(folder, "test.txt");
		File small = new File(folder, "C-small-attempt0.in");
		File large = new File(folder, "C-large.in");
	...
	static ContestProblem codeJam2012_Qualification_C = new ContestProblem() {

		@Override
		public void parseInput(Input in, PrintWriter out) {
			int cases = in.readInt();
			for (int caseNum = 1; caseNum <= cases; caseNum++) {
				List<String> input = in.readStrList();
				String strA = input.get(0);
				String strB = input.get(1);
				int a = Integer.parseInt(strA);
				int b = Integer.parseInt(strB);
				Set<List<Integer>> distinctPairs = new HashSet<>();

				for (int i = a; i <= b; i++) {
					Set<Integer> recycledNums = new HashSet<>();
					recycledNums.addAll(recycle(i));
					for (Integer n : recycledNums) {
						for (Integer m : recycledNums) {
				...
				/*
				 * System.out.format("%d -> %d%n", a, b); for (List<Integer> pair : distinctPairs) {
				 * System.out.println(Arrays.toString(pair.toArray(new Integer[0]))); }
				 */
				out.print(String.format("Case #%d: %s%n", caseNum, distinctPairs.size()));
			}
		}

		Set<Integer> recycle(int num) {
			String str = Integer.toString(num);
			Set<Integer> dump = new HashSet<>();
			for (int i = 0; i < str.length(); i++) {
				int r = Integer.parseInt(str.substring(i) + str.substring(0, i));
				dump.add(r);
			}
			return dump;
		}
