...
public class Main {
    ...
    public static void sovleDWG()
    {
        try{
            freader = new Scanner(
                    new File("C:/Users/Ben/Documents/NetBeansProjects/CodeJam/src/1a2011/B-small-attempt0.in"));
            caseCount = freader.nextInt();
            freader.nextLine();
    ...
    {
        int minSuprisingScore = p + 2*max(p-2,0);
        int minNonSuprisingScore = p + 2*max(p-1,0);
        int count = 0;
        int maybe = 0;
        ...
        for(int i = 0; i < n; i++)
        {
            ...
            if(curr >= minNonSuprisingScore)
            {
                count++;
            }
            else if(curr < minNonSuprisingScore && curr >= minSuprisingScore)
            {
                maybe++;
            }
        }
