...
public class C
{
	...
	//static String PROBLEM_NAME = "c-small-4";
	//static String PROBLEM_NAME = "c-large-0";	
	public static int[][] recycles = new int[2000000][7];
	public void solveCase() throws Exception 
	{
		...
		for(int i=A; i<=B; i++)
			for( int j=0; j<7; j++)
				if( recycles[i][j] <= B && recycles[i][j] >= A)
					cnt++;
	...
	public static void main(String[] args) throws Exception {
		...
		for(int i=1; i<2000000; i++)
		{
			...
			{
				String cycled = str.substring(len-j ) + str.substring(0, len-j );
				Integer integer = Integer.valueOf(cycled);
				int val = integer.intValue();
			
				if( val > i )
				{
					if( dups.contains(integer) == false )
					{
						dups.add(integer);
						recycles[i][j] = val;
					}
	...
	public static double[] converDoubletArray(ArrayList<Double> n){double[] ret=new double[n.size()];for(int i=0;i<ret.length;i++){if(n.get(i)!=null)ret[i]=n.get(i).doubleValue();else ret[i] = 0;} return ret;}
