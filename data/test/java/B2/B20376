...
//			stopTimer();
	...
	int cnt(int x, int a, int b) {
		String s = str(x);
		int r = 0;
		out: for (int i=1; i<s.length(); ++i) {
			int z = Integer.parseInt(s.substring(s.length()-i)+s.substring(0,s.length()-i));
			p[i] = z;
			for (int j=1; j<i; ++j) if (p[j]==z) continue out;
			if (z>=a && z<=b && x<z) r++;
		}
		return r;
	}
	...
	public static void main(String[] args) throws Exception {
		...
		if (!intoFile) {
			...
			File f = new File(System.getProperty("user.dir")+"\\src\\"+C.class.getPackage().getName()+"\\"+C.class.getSimpleName()+".out" );
